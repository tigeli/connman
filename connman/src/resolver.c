...
int __connman_resolver_redo_servers(int index)
{
	...
	/*
	 * We want to re-add all search domains back.
	 * Removal on search domains is not necessary as there
	 * can be only one instance of each search domain in the
	 * each dns-servers search domain list.
	 */
